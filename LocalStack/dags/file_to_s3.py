...
def file_to_s3():
    ...
    def modify_by_spark():
        ...
        for file in os.listdir(months_folder):
            if file.endswith(".csv"):
                month_header = file.replace("_date.csv","")
                ...
                output_dir = os.path.join(processed_path, month_header)
                result.coalesce(1).write.mode("overwrite").csv(output_dir, header=True)
    ...
    # UPLOAD 1 FILE TO S3. THE LOOP HAS BEEN SET UP FOR CASE OF MORE THAN 1 FILE
    @task
    def upload_by_spark():
        hook = S3Hook(aws_conn_id="localstack_s3")
    ...
    def to_dynamodb():
        ...
        def write_to_dynamo(partition):
            ...
            with table_original.batch_writer() as batch: # send rows by batches 
                for row in partition:
                    r = row.asDict() # assign Spark row to Python dict
                    try:
                        batch.put_item(
                            Item={
                                "ride_id": f"{Decimal(str(r.get('avg_speed (km/h)') or 0))}_{str(row['return_id'])}_{str(row['departure'])}",
